...
class DataValidation:
        ...
        try:
            ...
            for column in numerical_columns:
                if column not in dataframe.columns:
                    logging.info(f"Column missing: {column}")
                    return False
    ...
    def initiate_data_validation(self)-> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = "Train dataframe contains non-numerical columns."
                raise Exception(error_message)

            # Validate test data
            status = self.validate_numerical_columns(test_dataframe)
            if not status:
                error_message = "Test dataframe contains non-numerical columns."
                raise Exception(error_message)
